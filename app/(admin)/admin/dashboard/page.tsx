...
const STATUS_STYLE: Record<string, { label: string; color: string; bg: string }> = {
  ...
  FULFILLED:  { label: "Fulfilled",    color: "#5B21B6", bg: "#EDE9FE" },
};
    ...
    <div>
      ...
      >
        ...
        {recentOrders.length === 0 ? (
          <p className="px-6 py-10 text-sm text-center" style={{ color: "var(--brand-muted)" }}>
            No orders yet.
          </p>
        ) : (
          recentOrders.map((order, i) => {
            const statusStyle =
              STATUS_STYLE[order.status] ?? { label: order.status, color: "#888", bg: "#F3F4F6" };
            return (
              <Link
                key={order.id}
